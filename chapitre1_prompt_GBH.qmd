# QDA Data — Characterization of the Sensory Space
...
Before using the **Rj Editor** for the first time, open its settings using the gear icon and change the default R version (**jamovi R**) to **System R**. The system R installation must contain the packages required for the analysis — here `SensoMineR` — as well as `jmvconnect`, which allows the Rj Editor to access the dataset currently open in jamovi as `data`.

By checking **R code**, SEDA then displays an **R Code** output designed to be copied directly into the jamovi **Rj Editor**.
